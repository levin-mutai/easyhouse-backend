...
class myAccountManager(BaseUserManager):
    def create_user(self,email,username,fullname,phonenumber,password=None):
        if not email:
            raise ValueError('Users must have an email address')
        if not username:
            raise ValueError("users must have a username")
        if not fullname:
            raise ValueError("Users must have their full names")
        if not phonenumber:
    ...
    def create_superuser(self,email,username,fullname,phonenumber,password):
